...
def run_fix_mask_model_2(config, trainset= X_mnist_train, testset = X_mnist_test, critere = nn.MSELoss().to(device)):
    ...
    dir_name = f"mask_typ_{config.mask_typ}_mask_seed_{config.mask_seed}_mask_dim_{config.mask_dim[0]}_{config.mask_dim[1]}_spirit_bloc_{config.spirit_block}_kernel_{config.kernel1[0]}_{config.kernel1[1]}_lr_other_{config.lr_other}"
    ...
    print(f"mask_typ_{config.mask_typ}_mask_seed_{config.mask_seed}_mask_dim_{config.mask_dim}_spirit_bloc_{config.spirit_block}_lr_other_{config.lr_other}_kernel_{config.kernel1}")
    
    
    df = pd.DataFrame(columns=['mask_typ','mask_seed','mask_dim','bloc', 'kernel','lr_other','test_ssim','test_psnr','test_loss','test_loss_denorm'])
    ...
    line ={"mask_typ":config.mask_typ,"mask_seed":config.mask_seed ,"mask_dim":config.mask_dim ,"bloc" : config.spirit_block, "kernel" :config.kernel1,"lr_other": config.lr_other}
